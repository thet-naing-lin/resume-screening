        ...
        <div className="px-6 pt-6 pb-4">
          ...
          {!result && (
            <div className="flex items-center gap-2 text-xs font-semibold">
              <span className={`flex items-center gap-1.5 ${step === 1 ? "text-brand-600 dark:text-brand-400" : "text-surface-400"}`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold
                  ${step === 1 ? "bg-brand-500 text-white" : "bg-surface-100 text-surface-400 dark:bg-surface-800"}`}>1</span>
                Review Recipients
              </span>
              <span className="w-8 h-px bg-surface-200 mx-1 dark:bg-surface-700" />
              <span className={`flex items-center gap-1.5 ${step === 2 ? "text-brand-600 dark:text-brand-400" : "text-surface-400"}`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold
                  ${step === 2 ? "bg-brand-500 text-white" : "bg-surface-100 text-surface-400 dark:bg-surface-800"}`}>2</span>
                Compose & Send
              </span>
            </div>
          )}
                  ...
                  <div className="space-y-2 mb-5">
                    {recipients.map((r) => (
                      <div key={r.resume_id}
                        ...
                        <span className="text-sm font-medium text-surface-800 w-36 truncate dark:text-surface-200">{r.candidate_name}</span>
                        <input
                          type="email"
                          ...
                          className={`flex-1 border rounded-xl px-3 py-2 text-sm focus:outline-none
                                      ...
                                      ${!r.stored_email ? "border-red-300 bg-red-50 dark:bg-red-900/20 dark:border-red-700" : "border-surface-200 dark:border-surface-700"}`}
                        />
                        {!r.stored_email && (
          ...
          {!result && step === 2 && (
            <>
              <div className="space-y-4 mb-5">
                <div>
                  <label className="block text-sm font-medium text-surface-700 mb-1.5 dark:text-surface-300">Subject</label>
                  <input type="text" value={subject} onChange={(e) => setSubject(e.target.value)}
                         className="input-field" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-surface-700 mb-1.5 dark:text-surface-300">Body</label>
            ...
            <div className="animate-scale-in">
              <div className="flex items-start gap-3 bg-emerald-50 border border-emerald-200
                ...
                <div>
                  ...
                  {result.sent_count && (
                    <p className="text-sm text-emerald-600 mt-0.5 dark:text-emerald-400">{result.sent_count} email(s) sent</p>
                  )}
                </div>
              </div>
              ...
              {result.failed?.length > 0 && (
                <div className="mb-4">
                  <p className="text-sm font-semibold text-red-600 mb-2 dark:text-red-400">Failed ({result.failed.length}):</p>
                  <div className="space-y-1.5">
                    {result.failed.map((f, i) => (
                      <div key={i}
                           className="flex items-center gap-3 bg-red-50 border border-red-100 rounded-xl px-4 py-2.5 text-sm
                                      dark:bg-red-900/20 dark:border-red-800">
                        <span className="font-medium text-surface-700 dark:text-surface-200">{f.candidate_name}</span>
                        <span className="text-red-500 text-xs dark:text-red-400">— {f.reason}</span>
                      </div>
                    ))}
                  </div>
                </div>
